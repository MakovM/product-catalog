...
from catalog.models import Category, Product, CartItem, Cart
...
class CartSessionViewTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.category = Category.objects.create(name="Tablets", slug="tablets")
        self.product = Product.objects.create(
...
class CartDatabaseViewTest(TestCase):
    def setUp(self):
    ...
    def test_add_to_cart_view(self):
        url = reverse("products:add_to_cart", kwargs={"product_id": self.product.id})
    ...
    def test_remove_item_from_cart(self):
        self.client.post(reverse("products:add_to_cart", kwargs={"product_id": self.product.id}))
        self.client.post(reverse("products:add_to_cart", kwargs={"product_id": self.second_product.id}))

        self.client.post(reverse("products:cart_remove", kwargs={"product_id": self.product.id}))

        cart = Cart.objects.get(user=self.user)
        items = cart.items.all()

        self.assertEqual(items.count(), 1)
        self.assertEqual(items.first().product, self.second_product)
    ...
    def test_increase_and_decrease_item_quantity(self):
        ...
        self.client.post(reverse("products:cart_increase", kwargs={"product_id": self.product.id}))
        item.refresh_from_db()
        self.assertEqual(item.quantity, 2)

        self.client.post(reverse("products:cart_decrease", kwargs={"product_id": self.product.id}))
        item.refresh_from_db()
